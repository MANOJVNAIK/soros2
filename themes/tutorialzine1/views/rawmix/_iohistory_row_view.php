...
$setPoint = unserialize($Ar["rm_set_points"]);
...
if (count($spList) > 0) {
    foreach ($spList as $spAr) {
        ...
        if (isset($averagesAr[$spName])) {
            $spPointValue = $setPoint[$spName];
            $inAnyValsText .= '<td class="ui-state-default">' .
                    '<span style="font-weight:bold;" >' . round($averagesAr[$spName], 3) . '</span>' .
                    '<br/>' .
                    '<span>(' . round($spPointValue, 2) . ')</span></td>';
        } else
            $inAnyValsText .= '<td>-</td>';
    }//foreach
}//if count
...
if ($i % 2 == 0) {
    echo '<tr class="gradeA odd" style="background:#FFDDDD;">';
} else {
    echo '<tr class="gradeB even" style="background:#FF0000;">';
}

if ($_REQUEST["debug"] == 1)
    echo '<td > <a href="#" onclick="fetchErrorMsg(\'' . $Ar["rm_log_id"] . '\')">' . ($timeStamp) . '</a></td>';
else
    echo '<td style="width:100px" > <a href="#" >' . ($timeStamp) . '</a></td>';
echo '<td >' . $runType . '</td>';
echo $inFeedValsText;
echo $inAnyValsText;
